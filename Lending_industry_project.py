...
def credit_bureau_intro():
    ...
    st.subheader("1️⃣ What is a Credit Bureau?")
    st.write("""
...
def credit_bureau_data_tables():
    st.subheader("2️⃣ Data Received from Credit Bureau")

    st.markdown("### 📊 Credit Bureau Data Categories")
...
def project_overview():
    st.subheader("3️⃣ Project: Credit Default Prediction Using Bureau Data")
...
def preprocessing_details():
    ...
    **Transformation Applied:**
    ...
    - Mapped values such as `000 → 0`, `030 → 30`, `060 → 60`, `090 → 90+`

    **Derived Variables:**
    - `max_dpd_ever` → Maximum delinquency observed across history
    - `cnt_30plus` → Total count of 30+ DPD occurrences
    - `recent_6m_30plus` → Count of 30+ DPD in last 6 months (recency indicator)
    """)
    ...
    **Variables Aggregated:**
    - `total_loans` → Count of all credit trade-lines
    - `active_loans` → Count of currently active loans (`CREDIT_ACTIVE`)
    - `total_outstanding` → Sum of outstanding balances (`AMT_CREDIT_SUM_DEBT`)
    - `unsecured_cnt` → Count of unsecured loans (`CREDIT_TYPE`)
    - `max_dpd_ever` → Worst delinquency across all loans
    - `cnt_30plus` → Total delinquency count
    - `recent_6m_30plus` → Recent delinquency intensity
    """)
    ...
    **Variables Derived:**
    - `enquiry_cnt_6m` → Number of recent loan applications (credit hunger indicator)
    - `refused_cnt` → Count of previously refused applications (`NAME_CONTRACT_STATUS`)
    """)
...
def final_feature_list():
    st.subheader("4️⃣ Final Variables Used in Logistic Regression")
...
def train_test_info():
    st.subheader("5️⃣ Dataset Split")
...
def model_intro():
    st.subheader("6️⃣ Model Used: Logistic Regression")
...
def metrics_explanation():
    st.subheader("7️⃣ Model Evaluation Metrics")
